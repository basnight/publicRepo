...
**Doppler Integration**: The pipeline depends on a separate Doppler template (`livelife/io/digital-product/sre/cicd-tools/doppler`) included at the top of `.gitlab-ci-template.yml`. This provides the `secret` job that populates environment variables from Doppler into a `bash.env` artifact.
...
- `TIMESTAMP`: `YYYYMMDD-HHMM` in America/NewYork timezone
...
```yaml
include:
  - project: 'livelife/io/digital-product/sre/cicd-tools/cloud-deploy'
