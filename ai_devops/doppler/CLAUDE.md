...
```yaml
include:
  - project: 'livelife/io/digital-product/sre/cicd-tools/doppler'
...
2. **Project Path Mapping**: GitLab project paths are transformed to Doppler project names:
   - `livelife/io/digital-product/sre/cicd-tools/doppler` → `gitlab-sre-cicd-tools-doppler`
   - The prefix `livelife/io/digital-product/` is stripped, slashes become hyphens, and `gitlab-` is prepended

3. **Branch-to-Config Mapping**:
   - Development branches (`dev`, `develop`, `development-*`, `livelife/development`) → `dev` config
   - Staging branches (`integration`, `staging`, `staging-*`, `livelife/staging`) → `stg` config
   - Production branches (`main`, `master`, `livelife`, `production`, `production-*`) → `prd` config
